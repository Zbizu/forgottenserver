...
bool IOMapSerialize::saveHouseItems()
{
	...
	for (const auto& it : g_game.map.houses.getHouses()) {
		...
		for (HouseTile* tile : house->getTiles()) {
			...
			if (attributesSize > 0) {
				if (!stmt.addRow(std::format("{:d}, {:s}", house->getId(), db.escapeBlob(attributes, attributesSize)))) {
					return false;
				}
				stream.clear();
			}
...
bool IOMapSerialize::saveHouseInfo()
{
	...
	for (const auto& it : g_game.map.houses.getHouses()) {
		...
		if (result) {
			db.executeQuery(std::format("UPDATE `houses` SET `owner` = {:d}, `paid` = {:d}, `warnings` = {:d}, `name` = {:s}, `town_id` = {:d}, `rent` = {:d}, `size` = {:d}, `beds` = {:d} WHERE `id` = {:d}", house->getOwner(), house->getPaidUntil(), house->getPayRentWarnings(), db.escapeString(house->getName()), house->getTownId(), house->getRent(), house->getTiles().size(), house->getBedCount(), house->getId()));
		} else {
			db.executeQuery(std::format("INSERT INTO `houses` (`id`, `owner`, `paid`, `warnings`, `name`, `town_id`, `rent`, `size`, `beds`) VALUES ({:d}, {:d}, {:d}, {:d}, {:s}, {:d}, {:d}, {:d}, {:d})", house->getId(), house->getOwner(), house->getPaidUntil(), house->getPayRentWarnings(), db.escapeString(house->getName()), house->getTownId(), house->getRent(), house->getTiles().size(), house->getBedCount()));
		}
	}
	...
	for (const auto& it : g_game.map.houses.getHouses()) {
		...
		if (house->getAccessList(GUEST_LIST, listText) && !listText.empty()) {
			if (!stmt.addRow(std::format("{:d}, {:d}, {:s}", house->getId(), std::to_underlying(GUEST_LIST), db.escapeString(listText)))) {
				return false;
			}
		...
		if (house->getAccessList(SUBOWNER_LIST, listText) && !listText.empty()) {
			if (!stmt.addRow(std::format("{:d}, {:d}, {:s}", house->getId(), std::to_underlying(SUBOWNER_LIST), db.escapeString(listText)))) {
				return false;
			}
		...
		for (Door* door : house->getDoors()) {
			if (door->getAccessList(listText) && !listText.empty()) {
				if (!stmt.addRow(std::format("{:d}, {:d}, {:s}", house->getId(), door->getDoorId(), db.escapeString(listText)))) {
					return false;
				}
...
bool IOMapSerialize::saveHouse(House* house)
{
	...
	for (HouseTile* tile : house->getTiles()) {
		...
		const char* attributes = stream.getStream(attributesSize);
		if (attributesSize > 0) {
			if (!stmt.addRow(std::format("{:d}, {:s}", houseId, db.escapeBlob(attributes, attributesSize)))) {
				return false;
			}
			stream.clear();
		}
