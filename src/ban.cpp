...
bool IOBan::isAccountBanned(uint32_t accountId, BanInfo& banInfo)
{
	...
	DBResult_ptr result = db.storeQuery(std::format("SELECT `reason`, `expires_at`, `banned_at`, `banned_by`, (SELECT `name` FROM `players` WHERE `id` = `banned_by`) AS `name` FROM `account_bans` WHERE `account_id` = {:d}", accountId));
	if (!result) {
		return false;
	}
	...
	if (expiresAt != 0 && time(nullptr) > expiresAt) {
		...
		g_databaseTasks.addTask(std::format("INSERT INTO `account_ban_history` (`account_id`, `reason`, `banned_at`, `expired_at`, `banned_by`) VALUES ({:d}, {:s}, {:d}, {:d}, {:d})", accountId, db.escapeString(result->getString("reason")), result->getNumber<time_t>("banned_at"), expiresAt, result->getNumber<uint32_t>("banned_by")));
...
bool IOBan::isIpBanned(uint32_t clientIP, BanInfo& banInfo)
{
	...
	DBResult_ptr result = db.storeQuery(std::format("SELECT `reason`, `expires_at`, (SELECT `name` FROM `players` WHERE `id` = `banned_by`) AS `name` FROM `ip_bans` WHERE `ip` = {:d}", clientIP));
	if (!result) {
		return false;
	}
	...
	int64_t expiresAt = result->getNumber<int64_t>("expires_at");
	if (expiresAt != 0 && time(nullptr) > expiresAt) {
		g_databaseTasks.addTask(std::format("DELETE FROM `ip_bans` WHERE `ip` = {:d}", clientIP));
		return false;
	}
...
{
	return Database::getInstance().storeQuery(std::format("SELECT 1 FROM `player_namelocks` WHERE `player_id` = {:d}", playerId)).get() != nullptr;
}
