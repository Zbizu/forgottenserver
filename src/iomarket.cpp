...
MarketOfferList IOMarket::getActiveOffers(MarketAction_t action, uint16_t itemId)
{
	...
	DBResult_ptr result = Database::getInstance().storeQuery(std::format("SELECT `id`, `amount`, `price`, `created`, `anonymous`, (SELECT `name` FROM `players` WHERE `id` = `player_id`) AS `player_name` FROM `market_offers` WHERE `sale` = {:d} AND `itemtype` = {:d}", std::to_underlying(action), itemId));
	if (!result) {
		return offerList;
	}
...
MarketOfferList IOMarket::getOwnOffers(MarketAction_t action, uint32_t playerId)
{
	...
	DBResult_ptr result = Database::getInstance().storeQuery(std::format("SELECT `id`, `amount`, `price`, `created`, `itemtype` FROM `market_offers` WHERE `player_id` = {:d} AND `sale` = {:d}", playerId, std::to_underlying(action)));
	if (!result) {
		return offerList;
	}
...
HistoryMarketOfferList IOMarket::getOwnHistory(MarketAction_t action, uint32_t playerId)
{
	...
	DBResult_ptr result = Database::getInstance().storeQuery(std::format("SELECT `itemtype`, `amount`, `price`, `expires_at`, `state` FROM `market_history` WHERE `player_id` = {:d} AND `sale` = {:d}", playerId, std::to_underlying(action)));
	if (!result) {
		return offerList;
	}
...
void IOMarket::checkExpiredOffers()
{
	...
	g_databaseTasks.addTask(std::format("SELECT `id`, `amount`, `price`, `itemtype`, `player_id`, `sale` FROM `market_offers` WHERE `created` <= {:d}", lastExpireDate), IOMarket::processExpiredOffers, true);
...
{
	DBResult_ptr result = Database::getInstance().storeQuery(std::format("SELECT COUNT(*) AS `count` FROM `market_offers` WHERE `player_id` = {:d}", playerId));
	if (!result) {
		return 0;
	}
...
MarketOfferEx IOMarket::getOfferByCounter(uint32_t timestamp, uint16_t counter)
{
	...
	DBResult_ptr result = Database::getInstance().storeQuery(std::format("SELECT `id`, `sale`, `itemtype`, `amount`, `created`, `price`, `player_id`, `anonymous`, (SELECT `name` FROM `players` WHERE `id` = `player_id`) AS `player_name` FROM `market_offers` WHERE `created` = {:d} AND (`id` & 65535) = {:d} LIMIT 1", created, counter));
	if (!result) {
		offer.id = 0;
		offer.playerId = 0;
		return offer;
...
{
	Database::getInstance().executeQuery(std::format("INSERT INTO `market_offers` (`player_id`, `sale`, `itemtype`, `amount`, `price`, `created`, `anonymous`) VALUES ({:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d})", playerId, std::to_underlying(action), itemId, amount, price, time(nullptr), anonymous));
}

void IOMarket::acceptOffer(uint32_t offerId, uint16_t amount)
{
	Database::getInstance().executeQuery(std::format("UPDATE `market_offers` SET `amount` = `amount` - {:d} WHERE `id` = {:d}", amount, offerId));
}
...
{
	g_databaseTasks.addTask(std::format("INSERT INTO `market_history` (`player_id`, `sale`, `itemtype`, `amount`, `price`, `expires_at`, `inserted`, `state`) VALUES ({:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d})", playerId, std::to_underlying(type), itemId, amount, price, timestamp, time(nullptr), std::to_underlying(state)));
}
...
bool IOMarket::moveOfferToHistory(uint32_t offerId, MarketOfferState_t state)
{
	...
	DBResult_ptr result = db.storeQuery(std::format("SELECT `player_id`, `sale`, `itemtype`, `amount`, `price`, `created` FROM `market_offers` WHERE `id` = {:d}", offerId));
	if (!result) {
		return false;
	}
...
{
	DBResult_ptr result = Database::getInstance().storeQuery(std::format("SELECT `sale` AS `sale`, `itemtype` AS `itemtype`, COUNT(`price`) AS `num`, MIN(`price`) AS `min`, MAX(`price`) AS `max`, SUM(`price`) AS `sum` FROM `market_history` WHERE `state` = {:d} GROUP BY `itemtype`, `sale`", std::to_underlying(OFFERSTATE_ACCEPTED)));
	if (!result) {
		return;
	}
